...
class L_TV(nn.Module):
    ...
    def forward(self, image_A, image_B, fused):
        ...
        x1 = fused - intensity_joint
        temp1_1 = torch.cat((x1[:, :, 1:, :], x1[:, :, -1, :].unsqueeze(2)), 2)
        temp1_2 = torch.cat((x1[:, :, :, 1:], x1[:, :, :, -1].unsqueeze(3)), 3)
        # temp1 = torch.sqrt((x1 - temp1_1)**2) + torch.sqrt((x1 - temp1_2)**2)
        TV_loss  = F.mse_loss(x1, temp1_1) + F.mse_loss(x1, temp1_2)

        # x2 = fused - image_B
        # temp2_1 = torch.cat((x2[:, :, 1:, :], x2[:, :, -1, :].unsqueeze(2)), 2)
        # temp2_2 = torch.cat((x2[:, :, :, 1:], x2[:, :, :, -1].unsqueeze(3)), 3)
        # temp2 = (x2 - temp2_1)**2 + (x2 - temp2_2)**2
        return TV_loss
